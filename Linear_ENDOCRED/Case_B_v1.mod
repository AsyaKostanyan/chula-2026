...
C = 0*SIGNAL_CBP + (1-0.00)*C(-1) + eps_C; // Perfect Credibility
...
histval;
  ...
  DL_GDP(-1)    = 0.4; 
  DL_GDP(0)     = 0.2;  // GDPNow
  DL_GDP_BAR(0) = 2.6/4; // 0.53; // 
  g_GDP_BAR(0)  = 2.6/4; // 0.53; 
end;
...
writetable(T, 'caseB.xlsx');

// ===================== PDF OUTPUT =====================
exportgraphics(gcf, 'caseB.pdf', 'ContentType', 'vector');
