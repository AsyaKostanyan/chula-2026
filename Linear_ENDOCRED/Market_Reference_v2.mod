...
varexo eps_Y eps_PIE eps_RR_BAR eps_UNR_BAR eps_UNR_GAP eps_g_UNR
       ...
       eps_PIE4EXP20 eps_PIE4EXP40 eps_TERM10YR eps_RS;
...
RS = DELTA1*RS(-1) + (1 - DELTA1)*(RR_BAR + PIE4(+3) + DELTA2*(PIE4(+3) - PIE_SS) + DELTA3*Y) + eps_RS;
...
writetable(T, 'market_reference.xlsx');

// ===================== PDF OUTPUT =====================
exportgraphics(gcf, 'market_reference.pdf', 'ContentType', 'vector');
